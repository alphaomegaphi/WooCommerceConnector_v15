...
from .woocommerce_requests import (
    ...
    get_woocommerce_all_product_categories,
)
...
def sync_item_with_woocommerce(item, price_list, warehouse, woocommerce_item=None):
    ...
    erpnext_categories = frappe.db.sql(
        """
        WITH RECURSIVE category_path (item_group_name, PATH) AS
             (SELECT item_group_name,
                     parent_item_group
              FROM `tabItem Group`
              WHERE parent_item_group = ""
              UNION ALL SELECT g.item_group_name,
                               CONCAT(cpath.path, ',', g.item_group_name)
              FROM category_path AS cpath
              JOIN `tabItem Group` AS g ON cpath.item_group_name = g.parent_item_group)
           SELECT *
           FROM category_path
           WHERE concat(",", item_group_name) != PATH
           ORDER BY PATH
        """,
        as_list=True,
    )
    erpnext_categories_dict = dict(
        [(item[0], item[1].lstrip(",")) for item in erpnext_categories]
    )
...
def sync_item_image(item):
    ...
    extra_image_list = frappe.db.get_values(
        "File",
    ...
    if extra_image_list:
        ...
        frappe.log(
            f"Uploading an image list of {len(img_details)} images. {image_info=}"
        )
            ...
            if bin_since_last_sync > 0 or force != False:
                ...
                try:
                    ...
                    make_woocommerce_log(
                        ...
                        message=f"About to upload to {resource=}",
...
def load_price(item, debug=False):
    # find item in ERP
    erp_item_code = frappe.get_value(
        "Item", {"woocommerce_product_id": item.get("id"), "disabled": 0}, "name"
    )
    if erp_item_code:
